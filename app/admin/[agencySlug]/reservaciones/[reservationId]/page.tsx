...
import { getReservationContractPreparationStatus } from "@/lib/contracts/reservation-contract";
...
export default async function AdminReservationDetailPage({
  ...
  const reservationPaidInFull = financialSummary?.balance.fullyPaid === true;
  ...
  let ticketsByTraveler = new Map<string, Readonly<{ id: string; status: string; version: number; hasCredential: boolean }>>();
    ...
    if (!ticketReconciliationError) {
      ...
      for (const ticket of refreshed) {
        ...
        if (!current || ticket.status === "available" || (current.status !== "available" && ticket.version > current.version)) {
          const hasCredential = ticket.status === "available"
            ? await ticketRepository.hasActiveBoardingCredential({ agencyId: access.agency.agencyId, reservationId: reservation.id, travelerId: ticket.travelerId, ticketDocumentId: ticket.id })
            : false;
          ticketsByTraveler.set(ticket.travelerId, { id: ticket.id, status: ticket.status, version: ticket.version, hasCredential });
        }
      }
    }
  ...
  let contractPreparationStatus: Awaited<ReturnType<typeof getReservationContractPreparationStatus>> | null = null;
  if (!contractInstance) {
    try {
      contractPreparationStatus = await getReservationContractPreparationStatus({
        requestedAgencySlug: access.agency.agencySlug,
        reservationId: reservation.id,
      });
    } catch {
      contractPreparationStatus = { status: "invalid_structure" };
    }
  }
          ...
          <main className={detailStyles.adminDetailMain}>
            <section className={`${detailStyles.detailCard} ${detailStyles.reservationOverview}`} aria-labelledby="detail-trip-title">
              ...
              <div className={detailStyles.overviewGrid}>
                ...
                <dl>{reservation.primaryContact ? <><div><dt>Cliente</dt><dd>{valueOrUnavailable(reservation.primaryContact.fullName)}</dd></div><div><dt>Correo</dt><dd>{reservation.primaryContact.email ? <a href={`mailto:${reservation.primaryContact.email}`}>{reservation.primaryContact.email}</a> : "No disponible"}</dd></div><div><dt>Teléfono</dt><dd>{valueOrUnavailable(reservation.primaryContact.phone)}</dd></div></> : <div><dt>Cliente de la reservación</dt><dd>No disponible</dd></div>}<div><dt>Cuenta</dt><dd>{reservation.customerAccountLinked ? "Cuenta vinculada" : "Sin cuenta vinculada"}</dd></div></dl>
              </div>
            </section>

            <section className={detailStyles.detailCard} aria-labelledby="detail-travelers-title">
              <div className={detailStyles.detailCardHeader}><div><span className={detailStyles.sectionEyebrow}>Pasajeros</span><h2 id="detail-travelers-title">Viajeros</h2></div><p className={reservation.travelerDataStatus === "complete" ? detailStyles.summaryComplete : detailStyles.summaryPending} role="status">{reservation.travelerDataStatus === "invalid_structure" ? "Revisión requerida" : reservation.travelerDataStatus === "complete" ? `${completedTravelers} completos` : "Datos pendientes"}</p></div>
              {reservation.travelerDataStatus === "invalid_structure" && <p className={detailStyles.unavailable} role="alert">La estructura de viajeros no coincide con la ocupación contratada. Revisa los registros antes de continuar.</p>}
              {reservation.travelers.length ? <div className={detailStyles.travelersTableWrap}><table className={detailStyles.travelersTable}><thead><tr><th>Pasajero</th><th>Tipo</th><th>Estado</th></tr></thead><tbody>{reservation.travelers.map((traveler) => { const fullName = [traveler.firstName, traveler.lastName].filter(Boolean).join(" "); return <tr key={traveler.position}><td>{fullName || `Viajero ${traveler.position}`}</td><td>{traveler.travelerType === "adult" ? "Adulto" : "Menor"}</td><td><span className={traveler.status === "complete" ? detailStyles.inlineComplete : detailStyles.inlinePending}>{traveler.status === "complete" ? "Completo" : "Pendiente"}</span></td></tr>; })}</tbody></table></div> : <p className={detailStyles.unavailable}>No hay registros de viajeros disponibles.</p>}
            </section>
