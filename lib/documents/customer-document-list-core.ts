...
export type CustomerDocumentType = "payment_receipt" | "contract" | "voucher" | "ticket";
...
export type CustomerDocumentRow = Readonly<{ id: string; documentType: string; version: number; generatedAt: string; paymentId: string | null }>;
...
const types: readonly CustomerDocumentType[] = ["payment_receipt", "contract", "voucher", "ticket"];
const order: Record<CustomerDocumentType, number> = { ticket: 0, voucher: 1, contract: 2, payment_receipt: 3 };
  ...
  return { async list(input: Readonly<{ requestedAgencySlug?: string; reservationId: string }>): Promise<CustomerDocumentListResult> {
    ...
    try {
      ...
      const documents = rows.flatMap((row): CustomerDocumentItem[] => {
        ...
        if (row.documentType !== "payment_receipt") return [{ documentKey: row.id, documentType: row.documentType, version: row.version, generatedAt: row.generatedAt, paymentContext: null }];
        ...
        return payment && Number.isFinite(payment.amount) && isCurrency(payment.currency) && payment.paidAt
          ? [{ documentKey: row.id, documentType: row.documentType, version: row.version, generatedAt: row.generatedAt, paymentContext: { amount: payment.amount, currency: payment.currency, paidAt: payment.paidAt } }]
          : [];
