-- Hardens the already-applied historical traveler reconciler. An empty draft
-- must not UPDATE an already-existing empty pending row (and therefore must
-- not advance updated_at). Missing slots remain materialized for structure.
...
begin
  ...
  loop
    ...
    elsif first_name_value is null
      and last_name_value is null
      and birth_date_value is null
      and traveler_status = 'pending' then
      -- No source value or state advance: intentionally do not UPDATE, so the
      -- set_updated_at trigger cannot change updated_at.
      preserved_count := preserved_count + 1;
    else
