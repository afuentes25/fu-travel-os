...
export type AgencySettings = { visibleSections: string[]; modules: string[]; legalNotice: string };
...
export type DepartureBoardingOption = { id: string; departureId: string; agencyDeparturePointId: string; meetingTime: string; departureTime: string; surchargeAmount?: number; currency?: Currency; capacity?: number; availableSpaces?: number; bookingDeadline?: string; boardingOrder?: number; instructionsOverride?: string; status: "available" | "limited" | "sold_out" | "disabled" };
...
export type TravelExtra = { id: string; name: string; description?: string; price: number; currency: Currency; pricingMode: "per_person" | "per_booking" | "per_night"; optional: boolean };
...
export type TravelProduct = { id: string; agencyId: string; code: string; slug: string; title: string; subtitle?: string; summary: string; description: string; scope: GeographicScope; productType: TravelProductType; transportTypes: TransportType[]; tags: TravelThemeTag[]; region: TravelRegion; countries: string[]; cities: string[]; destinationIds: string[]; categoryIds: string[]; durationDays: number; durationNights: number; featuredImage: string; gallery: string[]; includes: string[]; excludes: string[]; requirements: string[]; recommendations: string[]; policies: TravelPolicies; itinerary: TravelItineraryDay[]; basePrice: TravelPrice; pricingOptions: TravelPricingOption[]; departures: TravelDeparture[]; extras: TravelExtra[]; status: "draft" | "review" | "published" | "archived"; featured: boolean; promotion?: string };
export type BookingBoardingSnapshot = { boardingOptionId: string; pointName: string; address: string; city: string; meetingTime: string; departureTime: string; surchargeAmount: number; currency: Currency; instructions?: string };
export type BookingStatus = "draft" | "pending" | "deposit_pending" | "partially_paid" | "confirmed" | "cancelled" | "refunded" | "completed";
export type CartLine = { id: string; agencyId: string; travelId: string; departureId: string; boardingOptionId: string; pricingOptionId: string; travelers: number; extraIds: string[] };
export type PricedCartLine = CartLine & { travel: TravelProduct; boarding: BookingBoardingSnapshot; subtotal: number; taxes: number; surcharge: number; extrasTotal: number; total: number; deposit: number };
